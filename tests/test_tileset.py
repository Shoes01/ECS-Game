...
if True:
    ...
    console = tcod.console_init_root(con_x, con_y, title='ECS Game', order='F', renderer=tcod.RENDERER_SDL2)
    ...
    im = Image.open('kenney_colored.png').convert('RGBA')
    nim = np.array(im)
    tx, ty, _ = nim.shape
    ...
    # Print something.
    count = 57344
    for y in range(0, con_y):
        if y % 2 == 0: continue

        for x in range(0, con_x):
            if x % 2 == 0: continue
            console.tiles["ch"][x][y] = count
            count += 1

    console.blit(console, 0, 0)
    tcod.console_flush()
    # print codepoint int at (x, y), print tile at (x+1, y)
        
    # Have a loop that's like
    # while console not closed
    # if kb input: close console
    cont = True
    while cont:
